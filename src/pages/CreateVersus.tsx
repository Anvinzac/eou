...
export default function CreateVersus() {
  ...
  const refreshPreview = (categoryId: string) => {
    const pool = academicData.questions.filter(q => q.category === categoryId);
    
    // Pick one random question from each difficulty
    const easy = pool.filter(q => q.difficulty === 'Easy').sort(() => Math.random() - 0.5)[0];
    const medium = pool.filter(q => q.difficulty === 'Medium').sort(() => Math.random() - 0.5)[0];
    const hard = pool.filter(q => q.difficulty === 'Hard').sort(() => Math.random() - 0.5)[0];
    
    setPreviewQuestions([easy, medium, hard].filter(Boolean));
  };
  ...
  const handleCreate = async () => {
    ...
    setIsCreating(true);
    try {
      // 1. Pick 5 random questions of the selected difficulty
      const pool = academicData.questions.filter(q => q.category === selectedCategory && q.difficulty === selectedDifficulty);
      if (pool.length === 0) throw new Error('No questions available for this difficulty');
      
      const shuffled = [...pool].sort(() => Math.random() - 0.5);
      const selectedQs = shuffled.slice(0, 5);
      
      // 2. Create quiz with title prefix [Versus]
      const title = `[Versus] ${selectedCategory} Challenge`;
      ...
      // 3. Create questions
      const questionsToInsert = selectedQs.map((q, i) => ({
        quiz_id: quiz.id,
